...
enum class OrderMethod { InOrder, PostOrder, PreOrder };
...
public:
  ...
  inline bool isEmpty() { return !root; }
  ...
  inline std::shared_ptr<BinaryNode> getRoot() { return root; }

  inline void
  loop(const OrderMethod method, std::shared_ptr<BinaryNode> troot,
       const std::function<void(const std::shared_ptr<BinaryNode>)> func) {
    switch (method) {
