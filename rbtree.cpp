...
std::shared_ptr<RbNode> RBTree::insertNode(std::shared_ptr<RbNode> root,
                                           std::shared_ptr<RbNode> ptr) {
  if (!root)
    return ptr;
...
void RBTree::fixInsert(std::shared_ptr<RbNode> ptr) {
  ...
  while (ptr != root && getColor(ptr) == Color::Red &&
         getColor(ptr->parent) == Color::Red) {
    parent = ptr->parent;
...
std::shared_ptr<RbNode> RBTree::deleteNode(std::shared_ptr<RbNode> root,
                                           const DsData data) {
  if (!root)
    return root;
...
void RBTree::inOrder(
    const std::shared_ptr<RbNode> ptr,
    const std::function<void(const std::shared_ptr<RbNode>)> func) {
  if (!ptr)
    return;
...
std::shared_ptr<RbNode>
RBTree::minValueNode(const std::shared_ptr<RbNode> node) {
  auto ptr = node;
  while (ptr->left)
    ptr = ptr->left;
...
std::shared_ptr<RbNode>
RBTree::maxValueNode(const std::shared_ptr<RbNode> node) {
  auto ptr = node;
  while (ptr->right)
    ptr = ptr->right;
